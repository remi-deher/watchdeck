...
<script setup lang="ts">import { computed,onMounted,ref } from "vue";import { Download } from "@lucide/vue";import { useRoute } from "vue-router";import { api } from "@/api";import UiButton from '@/components/ui/UiButton.vue';import UiEmptyState from '@/components/ui/UiEmptyState.vue';
...
const route=useRoute(),request=ref<MediaRequest|null>(null),releases=ref<Release[]>([]),loading=ref(false),grabbing=ref<string|null>(null),error=ref('');
...
async function load(): Promise<void> {loading.value=true;error.value='';try{request.value=await api(`/api/requests/${route.params.requestId}`);const p=new URLSearchParams({media_type:request.value!.media_type,request_id:String(request.value!.id)});releases.value=await api(`/api/arr/releases?${p}`)}catch(e:any){error.value=e.message;releases.value=[]}finally{loading.value=false}}
