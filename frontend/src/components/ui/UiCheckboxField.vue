<template>
  <div class="ui-checkbox-field" :class="{ 'has-error': error, 'is-disabled': disabled }">
    <Checkbox :input-id="inputId" binary :model-value="modelValue" :disabled="disabled"
      :aria-describedby="describedBy" :aria-invalid="error ? 'true' : undefined" @update:model-value="emit('update:modelValue', Boolean($event))" />
    <label :for="inputId">
