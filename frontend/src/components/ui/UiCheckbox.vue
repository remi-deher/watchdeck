...
<style scoped>
.ui-checkbox{display:inline-grid;flex:none;place-items:center;width:18px;height:18px;min-height:0;padding:0;border:1.5px solid var(--border-control);border-radius:var(--radius-xs);background:var(--surface-2);color:var(--on-accent);cursor:pointer;transition:background-color var(--motion-duration-instant) var(--motion-ease-standard),border-color var(--motion-duration-instant) var(--motion-ease-standard)}
...
.ui-checkbox-indicator :deep(svg){color:var(--on-accent);stroke-width:3.5}
