...
const global = {
  stubs: {
    ...
    InstanceOverviewGrid: {
      template: '<div class="instance-overview-stub" />',
    },
...
describe("DownloadsOverview", () => {
  it("synthétise la file, le stockage et les activités récentes", () => {
    const wrapper = mount(DownloadsOverview, {
      props: {
        queue: [
          { id: 1, title: "Film bloqué", status: "error", instance: "Radarr" },
          { id: 2, title: "Actif", status: "downloading" },
        ],
    ...
    expect(
      wrapper
        .findAll(".health-kpi-grid .metric-card strong")
        .map((node) => node.text()),
    ).toEqual(["2", "1", "0", "75 %"]);
