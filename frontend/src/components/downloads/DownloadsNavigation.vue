          ...
          <div v-if="openGroups.arr" class="arr-links">
            <div v-for="group in arrGroups" :key="group.key" class="arr-kind">
              <div class="source-parent arr-kind-parent">
                <RouterLink :to="target(group.key, group.items.length === 1 ? group.items[0].id : '')" :class="{ active: section === group.key && (!selectedSourceId || group.items.length === 1) }" active-class=""><component :is="group.icon" /><span class="source-label">{{ group.label }}</span><span class="nav-count">{{ group.items.length }}</span></RouterLink>
        ...
        <div v-for="group in clientGroups" :key="group.key" class="source-group">
          <div class="source-parent">
            <RouterLink
              ...
              active-class=""
            >
              <component :is="group.icon" /><span class="source-label">{{ group.label }}</span><span class="nav-count">{{ group.items.length }}</span>
            </RouterLink>
...
.sidebar.collapsed .source-label{display:none}.sidebar.collapsed .source-parent{display:block}.sidebar.collapsed .source-parent>a:first-child{justify-content:center}.sidebar.collapsed .source-toggle,.sidebar.collapsed .source-add,.sidebar.collapsed .source-links,.sidebar.collapsed .source-empty,.sidebar.collapsed .nav-count{display:none}
