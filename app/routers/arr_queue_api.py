...
async def _compute_arr_download_queue(db: AsyncSession) -> list[dict]:
    ...
    results = await asyncio.gather(*(fetch_instance_queue(instance) for instance in instances), return_exceptions=True)
    records_by_instance = []
    ...
    for instance, result in zip(instances, results):
        ...
        lookup_keys.update(
            (instance.id, int(record["arr_media_id"])) for record in result if record.get("arr_media_id") is not None
        )
    ...
    requests = (
        (
            await db.execute(
                select(MediaRequest).filter(tuple_(MediaRequest.arr_instance_id, MediaRequest.arr_id).in_(lookup_keys))
            )
        )
        .scalars()
        .all()
    ...
    library_items = (
        (
            await db.execute(
                select(LibraryItem).filter(tuple_(LibraryItem.arr_instance_id, LibraryItem.arr_id).in_(lookup_keys))
            )
        )
        .scalars()
        .all()
